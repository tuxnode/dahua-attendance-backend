...
服务接收门禁设备主动上报的通行记录、门状态和抓拍事件，解析后写入 MySQL，并通过 Gin HTTP API 为前端提供考勤记录、日报、月报、汇总和异常列表查询。Nacos 注册为可选能力。
...
- 考勤统计：支持日报、月报、汇总、异常列表。
- 考勤规则：支持业务时区、上下班时间、周末/节假日、工作日覆盖、弹性打卡、多班次和排班。
